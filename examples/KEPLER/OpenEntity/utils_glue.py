...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id, pos):
...
def convert_examples_to_features(examples, label_list, max_seq_length,
    ...
    def getSp(sent,s,t):
        ...
        for i in range(0,len(tokens)):
            if tokens[i]==rs[0] and i+len(rs)<=len(tokens):
                Flag=False
    ...
    def getIns(bped,bpeTokens,tokens,L,R):
            ...
            resR=0
            tkR=" ".join(tokens[R:])
            bped_tkR=" ".join(tokenizer.tokenize(tkR))
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        raw_tokens,eL,eR=getSp(ex_text_a,h[1],h[2])
        s=" ".join(raw_tokens)
        sst=tokenizer.tokenize(s)
        eL, eR=getIns(" ".join(sst),sst,raw_tokens,eL,eR)
        sst.insert(eL,'madeupword0000')
        sst.insert(eR+1,'madeupword0001')
        tokens_a=sst
        pos=eL+1 
        ...
        features.append(
                InputFeatures(input_ids=input_ids,
                              ...
                              label_id=label_id,
                              pos=pos+1 if pos+1 <max_seq_length else pos))
    return features
