...
class RobertaForTyping(BertPreTrainedModel):
    r"""
        **labels**: (`optional`) ``torch.LongTensor`` of shape ``(batch_size,)``:
            Labels for computing the sequence classification/regression loss.
            Indices should be in ``[0, ..., config.num_labels]``.
    ...
    def __init__(self, config):
        super(RobertaForTyping, self).__init__(config)
        self.num_labels = config.num_labels

        self.roberta = RobertaModel(config)
        self.typing = RobertaClassificationHead(config)
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        outputs = self.roberta(input_ids,
                               ...
                               head_mask=head_mask)
        sequence_output = outputs[0]
        #tmp=pos.unsqueeze(2).float()
        #sequence_output = (tmp * outputs[0]).sum(1)
        logits = self.typing(sequence_output, pos)
